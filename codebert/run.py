...
Removed all models but CodeBERT.
"""
...
class TextDataset(Dataset):
    """A PyTorch dataset from a text file."""
    ...
    def __getitem__(self, i):
        input_ids, label = self.examples[i].input_ids, self.examples[i].label
        return torch.tensor(input_ids), torch.tensor(label)


def set_seed(seed=42):
    """Set seed of all imaginable RNGs"""
    random.seed(seed)
...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(bar):
            ...
            # Evaluate model and save if it's better than the previous best
