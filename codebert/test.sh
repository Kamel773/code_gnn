...
python run.py --output_dir=./saved_models \
  ...
  --train_data_file=../data/devign/train.jsonl --eval_data_file=../data/devign/valid.jsonl --test_data_file=../data/devign/test.jsonl \
  --epoch 5 --block_size 400 --train_batch_size 32 --eval_batch_size 64 --learning_rate 2e-5 --max_grad_norm 1.0 \
  --do_eval --do_test \
  --seed 123456
python evaluator.py -a ../data/devign/test.jsonl -p saved_models/predictions.txt
